...
class TypeScriptHeaderGenerator extends AbstractHeaderGenerator {
	...
	private static void generateDependentModule(EntityWithModuleMetaData<ModuleNode> module, File outputDirectory, String header, boolean generateAccessor) {
		...
		if (moduleLang == DefinitionLanguage.Spaghetti) {
			...
			generateTypeScriptDependentModule(module, outputDirectory, header, generateAccessor);
		} else {
	...
	private static void generateTypeScriptDependentModule(EntityWithModuleMetaData<ModuleNode> module, File outputDirectory, String header, boolean generateAccessor) {
		def lines = []
		def namespace = module.entity.name
		lines.add("module ${namespace} {")
		lines.add("\tdeclare var ${SPAGHETTI_CLASS}: any;")
		def accessor = GeneratorUtils.createModuleAccessor(namespace, module.format)
		lines.add("\t${namespace} = ${accessor};")
		lines.add("}")
