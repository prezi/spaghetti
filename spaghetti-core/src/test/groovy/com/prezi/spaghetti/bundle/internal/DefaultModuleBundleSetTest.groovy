...
class DefaultModuleBundleSetTest extends Specification {
	def "empty"() {
		def set = new DefaultModuleBundleSet([] as Set, [] as Set, [] as Set)
	...
	def "mixed"() {
		...
		def set = new DefaultModuleBundleSet([direct1, direct2] as Set, [] as Set, [transitive1, transitive2] as Set)
	...
	def "not disjoint"() {
		...
		when:
		new DefaultModuleBundleSet([bundle] as Set, [] as Set, [bundle] as Set)
	...
	def "external dependency collection"() {
		...
		when:
		def set = new DefaultModuleBundleSet([bundleA, bundleB], [] as Set, [] as Set)
